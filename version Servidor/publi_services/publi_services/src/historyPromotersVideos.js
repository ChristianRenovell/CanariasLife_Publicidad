...
function querrysGett() {
    return new Promise(async function (resolve, reject) {
        var query = await connection.query('SELECT * FROM `promoters` WHERE nameVideo != ""', function (err, rows) {
            if (err) {
                return reject(err)
            }
            resolve(rows);
...
function querrysSett(result) {
    return new Promise(async function (resolve, reject) {
        ...
        for (let y = 0; y <= result.length; y++) {
            var query2 = await connection.query(`INSERT INTO historypromotersvideo ( id, name, date)  VALUES ('${result[y].id}','${result[y].nameVideo}','${grupDateHours}')`, function (error, result2) {
                if (error) {
                    return reject(err)
                } else {
                    resolve("ok");
